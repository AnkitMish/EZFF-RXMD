...
real(8),allocatable :: pos0(:,:), pos1(:,:), rr3(:,:)
character(3),allocatable :: ctype0(:), ctype1(:)
integer,allocatable :: itype0(:)
real(8),allocatable :: itype1(:), charge(:)
...
if(getReal) then !--- from normalized to real coordinates
  ...
  do k=0,mc(3)-1
     do n=1,natoms
        ...
        write(40,'(a3,1x,4f15.9)') ctype(n),rr(1:3), charge(n)
     enddo
  enddo; enddo; enddo
...
allocate(rr3(3,natoms))
do i=1, natoms
        rr3(1,i)=sum(H(1,1:3)*pos1(1:3,i))
        rr3(2,i)=sum(H(2,1:3)*pos1(1:3,i))
        rr3(3,i)=sum(H(3,1:3)*pos1(1:3,i))
enddo

!L1 = L1+100.0
!L2 = L2+100.0
!L3 = L3+100.0
rr3(1,1:natoms) = rr3(1,1:natoms) + 0.0
rr3(2,1:natoms) = rr3(2,1:natoms) + 0.0
rr3(3,1:natoms) = rr3(3,1:natoms) + 0.0
call getbox(L1,L2,L3,lalpha,lbeta,lgamma)
call matinv(H,Hi)

do i=1, natoms
        pos1(1,i)=sum(Hi(1,1:3)*rr3(1:3,i))
        pos1(2,i)=sum(Hi(2,1:3)*rr3(1:3,i))
        pos1(3,i)=sum(Hi(3,1:3)*rr3(1:3,i))
enddo
...
!--- each atom has 32 (=3*8 + 8) bytes data
   ...
   write(1,pos=ii*40+1) pos1(1:3,n),itype1(n), charge(n)
...
do myid=0,nprocs-1
   ...
   do n=1,lnatoms(myid)
      ...
      write(20,'(4f12.5)') rr(1:3), charge(n)
   enddo
